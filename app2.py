...
def process():
    ...
    for sheet_name in excel_file.sheet_names:
        ...
        #newdict[sheet_name]=[total_students,passed_students,backlogs]
    ...
    {sheet_name: results[sheet_name]['overall'][sheet_name][:3] for sheet_name in results.keys()},
    orient='index', columns=['Total Students', 'Passed Students', 'Backlogs'])'''
    new_df = pd.DataFrame.from_dict(
...
# Create a figure and axis
    ...
    positions = range(len(new_df.index))
    
    # Plot the bars for Total Students
    ax.bar(positions, new_df['Total Students'], bar_width, label='Total Students', color='blue')
    
    # Plot the bars for Passed Students
    ax.bar([p + bar_width for p in positions], new_df['Passed Students'], bar_width, label='Passed Students', color='green')
    
    # Plot the bars for Backlogs
    ax.bar([p + 2 * bar_width for p in positions], new_df['Backlogs'], bar_width, label='Backlogs', color='red')
    ...
    # Set the title
    ax.set_title('Student Statistics')
