...
def process():
    ...
    # Generate bar plot for overall data
        '''df_overall = pd.DataFrame.from_dict(
            {(sheet_name, sheet_name): results[sheet_name]['overall'][sheet_name] for sheet_name in results.keys()},
    ...
    '''df_overall = pd.DataFrame.from_dict(
        {(sheet_name, sheet_name): results[sheet_name]['overall'][sheet_name] for sheet_name in
         results.keys()}, orient='index', columns=['Total Students', 'Passed Students', 'Backlogs', 'Distinction',
                                                   'First Class', 'Second Class', 'Pass Class', 'Pass Percentage'])'''
    ...
    new_df = df_overall.loc[:, ['Total Students', 'Passed Students', 'Backlogs']]
